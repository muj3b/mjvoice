...
fragment float4 rainbow_fragment(VertexOut in [[stage_in]],
    ...
    float baseThickness = mix(0.004, 0.022, uniforms.intensity);
    float glowThickness = baseThickness * 2.6;
    float highlightThickness = baseThickness * 0.45;
    float softness = baseThickness * 0.45;
    ...
    if (uniforms.rippleProgress >= 0.0) {
        ...
        float width = max(uniforms.rippleWidth, 0.001);
        float envelope = exp(-(distance * distance) / (2.0 * width * width));
        rippleMod = envelope * uniforms.rippleStrength;
        // Pull the band towards the center to create a warp effect.
        edgeDistance = max(edgeDistance - rippleMod, 0.0);
    }
    ...
    float hue = fract(uniforms.time * 0.08 + uv.x * 0.6 + uv.y * 0.35);
    float3 rainbow = hsv2rgb(float3(hue, 0.85 + 0.1 * uniforms.intensity, 0.85));
    float3 glowColor = rainbow * (0.6 + 0.6 * uniforms.intensity + rippleMod * 1.6);
    float3 highlightColor = float3(1.0) * (0.08 + 0.32 * uniforms.intensity + rippleMod * 0.6);

    float alpha = edgeMask * (0.55 + 0.4 * uniforms.intensity) + glowMask * 0.35;
